...
class ProductController extends Controller
{
    ...
    public function postUpdateProduct(Request $request)
    {
        ...
        if (!$product && $input['updated_at']) {
            return redirect()->back()->withErrors(['error' => 'Không tìm thấy sản phẩm.']);
        }

        // Kiểm tra nếu sản phẩm đã bị thay đổi ở nơi khác
        if (isset($input['updated_at']) && $product->updated_at != $input['updated_at']) {
            return redirect()->back()->withErrors([
                'error' => 'Sản phẩm đã được cập nhật ở tab khác. Vui lòng tải lại trang để tiếp tục.'
            ]);
        }
